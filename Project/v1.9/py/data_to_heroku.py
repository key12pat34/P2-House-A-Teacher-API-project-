...
cur.execute(
    """
    CREATE TABLE IF NOT EXISTS as marker_data
    (SELECT schooltable._id, schooltable.name, schooltable.schooltype, schooltable.enrollment, schooltable.teachercount, location.longitude, location.latitude
    FROM schooltable
    FULL OUTER JOIN location ON schooltable._id=location._id
    ORDER BY schooltable._id)
    PRIMARY KEY (schooltable._id);
     """)
